...
def meters_to_latlong(lat, lon, meters):
    # Calculate the latitude/longitude equivalent of the distance in meters
        ...
        for i in range(max(-row, -max_distance_cells), min(height - row, max_distance_cells + 1)):
            for j in range(max(-col, -max_distance_cells), min(width - col, max_distance_cells + 1)):
                ...
                if distance <= 1:  # Normalized distance
                    decay = 1 - distance  # Linear decay
                    #decay = 1 / (2 ** (distance * (influence_meters / unit_size)))
                    grid[row + i, col + j] += decay
