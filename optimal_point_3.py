...
def objective_function(x, points, center, radius, public_road_lines):
    point = Point(x[1], x[0])
    # Check if the point is on a public road
    if not any(line.distance(point) < 0.001 for line in public_road_lines):  # Allow a small buffer for proximity to roads
        return 1000  # Penalize points not on public roads with a large penalty value
...
def get_public_road_lines(center, radius):
    # Get OSM data for the area within the radius from the center
    tags = {'highway': True}  # Get all types of roads
    ...
    public_road_lines = [geom for geom in gdf['geometry'] if isinstance(geom, LineString)]
    return public_road_lines

def precompute_optimal_points(points, center, radius_range, step, public_road_lines):
    optimal_points = {}
    ...
    for radius in np.arange(radius_range[0], radius_range[1] + step, step):
        ...
        result = differential_evolution(objective_function, bounds, args=(points, center, radius, public_road_lines),
...
def main():
    parser = argparse.ArgumentParser(description="Find the optimal point within a bounding circle that is furthest from a set of coordinates and on public roads.")
    ...
    # Get public road lines from OpenStreetMap
    public_road_lines = get_public_road_lines(center, max(radius_range))

    # Precompute optimal points for the range of radii
    optimal_points = precompute_optimal_points(points, center, radius_range, step, public_road_lines)
